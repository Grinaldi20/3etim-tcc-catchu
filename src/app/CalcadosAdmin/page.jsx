...
export default function CalcadosAdmin() {
...
useEffect(() => {
  async function carregarObjetosAdmin() {
    try {
      setLoading(true);
      // Buscar apenas objetos da categoria "Calcados" para classificar igual à tela pública
      const response = await api.get("/objetos", { params: { categ_nome: 'Calcados' } });
      ...
      const carrinho = storedCarr ? JSON.parse(storedCarr) : [];
