...
useEffect(() => {
  async function carregarObjetosAdmin() {
    try {
  // Buscar apenas objetos da categoria "Objetos gerais" para classificar igual à tela pública
  const response = await api.get("/objetos", { params: { categ_nome: 'Objetos gerais' } });
      ...
      // Ordena por data de publicação (mais recentes primeiro). Se a data for inválida, mantém posição.
      todos.sort((a, b) => {
...
function excluirItem(id) {
  ...
  // Nota: não chamamos listarObjetos() aqui para evitar duplicar fetch/estado
  // A carga de objetos é feita em carregarObjetosAdmin (acima) que depende de
  // `itemsRemovidos` e `finalizadosState` e roda no cliente após mount.
